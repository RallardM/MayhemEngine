...
namespace Mayhem
{
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		MAYHEM_ENGINE_ASSERT(false, "Unknown shader type!");
		return 0;
	}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			{
				MAYHEM_ENGINE_ERROR("Could not read from file '{0}'", filepath);
			}
		}
		else
		{
			MAYHEM_ENGINE_ERROR("Could not open file '{0}'", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			MAYHEM_ENGINE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
			std::string type = source.substr(begin, eol - begin);
			MAYHEM_ENGINE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
			MAYHEM_ENGINE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				MAYHEM_ENGINE_ERROR("{0}", infoLog.data());
				MAYHEM_ENGINE_ASSERT(false, "Shader compilation failure!");
				break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			MAYHEM_ENGINE_ERROR("{0}", infoLog.data());
			MAYHEM_ENGINE_ASSERT(false, "Shader link failure!");
			return;
		}
